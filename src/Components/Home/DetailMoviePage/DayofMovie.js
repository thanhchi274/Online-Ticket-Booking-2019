    ...
    if (this.props.movieDate) {
        ...
        if(renderNgayChieu.length ===0){
          return(
            ...
            <span>
            Hiện không có Lịch Chiếu cho phim này!
            </span>
            </div>
          )
        }
        else{
          return (renderNgayChieu.length!==0&&typeof(renderNgayChieu))==="object"?(
                ...
                <React.Fragment key={index}> {
                  Object.keys(renderNgayChieu[value].cumRapChieu).map((item1,indexTheater)=>{
                      ...
                      const duplicatedItem = arrayDuplicated.find(movie =>(new Date(movie.ngayChieuGioChieu).toLocaleDateString() === new Date(current.ngayChieuGioChieu).toLocaleDateString()))
                          ...
                          return (
                              <Tabs size="large" key={indexTheater} onChange={this.handleChange} defaultActiveKey="1" tabPosition={"top"} style={{ transform: `translate(0px, 0px)`}}>{
                              Object.keys(filteredArr).map((dateMovie,indexDateMovie)=>{
                                let time= new Date(filteredArr[dateMovie].ngayChieuGioChieu).toLocaleDateString()
                                let defaultTime = new Date(filteredArr[0].ngayChieuGioChieu).toLocaleDateString()
                               return ( 
                                   <TabPane key={time} tab={time}>
                                   <h5 className="ml-2 detailMovie--tenRap">{filteredArr[dateMovie].tenRap}</h5>
                                   <TimeofMovie LichChieu ={objLichChieu} defaultTime={defaultTime} ngayChieu={this.state.ngayChieu} />
                                   </TabPane>
                                      )})}
                                   </Tabs>
                            )})}
